...
private constructor(blockDevice: BlockDeviceDriver, first512Bytes: ByteBuffer) : FileSystem {
    ...
    private lateinit var _rootDirectory: FatDirectory
    ...
    override val volumeLabel: String
        get() {
            return _rootDirectory.volumeLabel?.let { it }.orEmpty()
        }
    ...
    init {
        fsInfoStructure = FsInfoStructure.read(
            blockDevice,
            bootSector.fsInfoStartSector * bootSector.bytesPerSector
        )
        fat = FAT(blockDevice, bootSector, fsInfoStructure)
        _rootDirectory = FatDirectory.readRoot(this, blockDevice, fat, bootSector)
